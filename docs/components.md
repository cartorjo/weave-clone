...
| Filter-Chip | `.filter-button`, `__check` — via `chip()` in content/render.mjs | Toggle-Chip der Projektfilter: Ruhe-Outline `--color-line-strong` (3:1), ausgewählt = Lemon + Ink-Outline + führender Haken, Hover (unausgewählt) nur Tönung. Werte ohne Referenz werden beim Build `disabled` gerendert (Owner 26.09., B-42): `--state-disabled` (38 %), kein Hover/Press, keine Tab-/Pfeiltasten-Station, `?branche=` ignoriert sie; mit der ersten passenden Referenz automatisch wieder aktiv. Auswahl steht in der URL (`?branche=` / `?leistung=`, replaceState; B-41). Eine Tab-Station pro Gruppe, Pfeiltasten/Home/End wechseln (js/06-work.js). |
