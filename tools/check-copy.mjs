...
// tools/copy-baseline.json. Order-only changes (moves) pass with a note;
// added or removed strings fail.
...
function strings(html) {
  ...
  for (const [, json] of html.matchAll(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g)) {
    const walk = v => { if (typeof v === 'string') push(v); else if (v && typeof v === 'object') for (const [k, x] of Object.entries(v)) if (!k.startsWith('@')) walk(x); };
...
const compare = (label, before = [], after = []) => {
  ...
  for (const s of added) failures.push(`${label}: added "${s.slice(0, 90)}"`);
