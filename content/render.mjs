...
export function disciplinePage(slug) {
  ...
  if (!d) throw new Error(`Unknown discipline: ${slug}`);
  const related = projects.filter(p=>p.discipline===slug);
...
function management() {
  // Photos where supplied; an initials tile for the one member without a portrait.
  // No roles or bios are invented — only the two supplied biographies render as profiles.
  const gallery = [
    {name:'Aleksandar Amidzic', image:'aleksandar-amidzic'},
    {name:'Markus Auer', image:'markus-auer'},
    {name:'Hans Lang', initials:'HL'},
    {name:'Dr. Michael Schmitt', image:'michael-schmitt'},
    {name:'Marcus Hefele', image:'marcus-hefele'},
  ];
  const tiles = gallery.map(person=>`<li>${person.image ? `<figure>${picture(person.image,'(max-width: 700px) 50vw, 20vw')}</figure>` : `<div class="management-grid__initials" aria-hidden="true">${person.initials}</div>`}<span>${escape(person.name)}</span></li>`).join('');
  return `<section class="page-section page-section--paper" id="management" aria-labelledby="management-title"><div class="gutter"><div class="container"><p class="eyebrow">Management</p><h2 class="page-title" id="management-title">Menschen, die Verantwortung übernehmen.</h2><div class="management-list"><article class="management-profile"><figure class="management-profile__media--wide">${picture('claus-thierbach','(max-width: 700px) 100vw, 25vw')}</figure><div><h3>Claus Thierbach</h3><p class="management-profile__role">Director Emposo Professional Partner Solutions</p><p>Der Diplom-Ingenieur für Maschinenbau startete seine Karriere 1996 im Anlagen- und später im Flugzeugbau. 2015 wurde Claus Thierbach Teil von Emposo. In Business Development, der Verantwortung für das operative Geschäft und beim Aufbau des Standortes in Rumänien hat er die Entwicklung des Unternehmens mitgestaltet.</p><p>Seit Juli 2026 verantwortet er die Businessline Emposo Professional Partner Solutions, das Geschäft mit Partnern und den Aufbau des Partnernetzwerkes.</p></div></article><article class="management-profile"><figure class="management-profile__media--wide">${picture('roman-bretz','(max-width: 700px) 100vw, 25vw')}</figure><div><h3>Roman Bretz</h3><p class="management-profile__role">Technischer Direktor Emposo</p><p>Roman Bretz begann seine Laufbahn 2001 bei Siemens Healthineers und gestaltete dort zuletzt als Systemarchitekt die Entwicklung von Krebstherapiezentren mit. Ab 2010 führte er bei LieberLieber Software als CTO ein neues Produkt für Systems Engineering zur Marktreife und beriet internationale Industriekunden. 2018 baute er ein Start-up für Explainable AI mit auf.</p><p>Seit 2021 ist er bei Emposo. Als Technischer Direktor verantwortet er das Lösungsportfolio über alle Business Lines und arbeitet mit den Teams an dessen Weiterentwicklung. Die Industrialisierung von KI und die digitale Transformation sind seine zentralen Themen, nach innen wie nach außen: Er treibt sie bei Emposo selbst voran und berät dazu die Kunden.</p><a class="text-link" href="https://www.linkedin.com/in/romanbretz">Roman Bretz auf LinkedIn ${arrow}</a></div></article></div><ul class="management-grid" aria-label="Weitere Mitglieder des Management-Teams">${tiles}</ul></div></div></section>`;
}
